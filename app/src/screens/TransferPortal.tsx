    ...
    <GameShell player={player}>
      <div className="flex flex-col items-center gap-1 text-center">
        ...
        <p className="text-sm font-bold text-white/85">
          {offers.length > 0
            ? `${player.collegeName} remains an option, but other programs are interested in ${player.name}.`
            : `No serious outside offers for now — ${player.collegeName} remains the best option.`}
        </p>
      </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        >
          ...
          <p className="mt-1 text-sm text-[var(--text-dim)]">
            {player.depthChartStatus === 'starter' ? 'Entrenched starter' : 'Keep proving yourself'}
          </p>
        </button>
        ...
        {offers.map((offer) => (
          ...
          >
            ...
            <p className="mt-1 text-sm text-[var(--text-dim)]">Tier {offer.programTier} program</p>
          </button>
        ))}
      </div>
    </GameShell>
